...
impl<W: Debug + Default + Clone + AddAssign<W> + SubAssign<W>, N: Debug + Default + Clone + AddAssign<N> + SubAssign<N>> GraphBuilder<W, N> {
...
// iterate over all non-empty nodes.
    ...
    {
        match op {
            EdgeOperation::SetEdgeWeight  {weight: w} => self.update_edge_weight(w, WeightUpdate::Set),
            EdgeOperation::IncEdgeWeight  {weight: w} => self.update_edge_weight(w, WeightUpdate::Inc),
            EdgeOperation::DecEdgeWeight  {weight: w} => self.update_edge_weight(w, WeightUpdate::Dec),

            EdgeOperation::SetNodeWeight {weight: w} => self.update_node_weight(w, WeightUpdate::Set),
            EdgeOperation::IncNodeWeight {weight: w} => self.update_node_weight(w, WeightUpdate::Inc),
            EdgeOperation::DecNodeWeight {weight: w} => self.update_node_weight(w, WeightUpdate::Dec),
...
/// creates a new edge with that weight.
    pub fn update_edge_weight(&mut self, weight: W, mode: WeightUpdate) {
        ...
        if let Some(edge_idx) = cur_edge {
            ...
            assert!(e.dst == to);
            match mode {
                WeightUpdate::Inc => {e.weight += weight;}
                WeightUpdate::Dec => {e.weight -= weight;}
                WeightUpdate::Set => {e.weight = weight;}
            }
        } else {
// Virtual edge. Create!
...
/// updates the node weight of the current to-node.
    fn update_node_weight(&mut self, weight: N, mode: WeightUpdate) {
        match mode {
            WeightUpdate::Inc => {self.nodes[self.current_state.to_node].weight += weight;}
            WeightUpdate::Dec => {self.nodes[self.current_state.to_node].weight -= weight;}
            WeightUpdate::Set => {self.nodes[self.current_state.to_node].weight = weight;}
        }
    }
...
/// Change from-node of current link to n-th input edge of current from node.  If no input edge
/// exists, the edge is left as is. TODO: Test case The n-th input node is not deleted.  NOTE: Does
/// not modify the graph itself, only changes the current link.
...
/// Copy all incoming edges of from-node into to-node, then replace from-node with to-node. The
/// current link is removed.  Do not create a self-loop for links between A and B.
...
#[cfg(test)]
fn edge_list<W: Debug + Default + Clone + AddAssign<W> + SubAssign<W>,
             N: Debug + Default + Clone + AddAssign<N> + SubAssign<N>>
...
fn test_update_edge_weight() {
    ...
    builder.update_edge_weight(4.0, WeightUpdate::Inc);
    assert_eq!(vec![vec![(0, 4.0)]], edge_list(&builder));

    builder.update_edge_weight(4.0, WeightUpdate::Dec);
    assert_eq!(vec![vec![(0, 0.0)]], edge_list(&builder));
}
...
fn test_next() {
    ...
    builder.update_edge_weight(1.0, WeightUpdate::Inc);
...
fn test_graph_paper() {
    ...
    // figure 2.a
    builder.update_edge_weight(0.25, WeightUpdate::Inc);
...
fn test_save_restore() {
    ...
    builder.update_edge_weight(0.7, WeightUpdate::Inc); // this will create a real edge.
